...
const Azar = ({hide, dark}) => {
    return (
        <Fragment>
            <div className={`absolute rounded-3xl z-10 left-17/100 top-23/100 ${dark ? "bg-white" : "dark-mode"} w-36 flex justify-around ${hide ? "hidden" : "flex"}`}>
                <table class="table-auto border-none mx-auto w-full mt-4">
                    <thead className=''>
                        <tr className='pb-8 leading-10'>
                            <th className={`text-xs font-light ${dark ? 'c-text-light' : 'text-white'}`}> آذربایجان غربی </th>
